---
title: Interactive
description: Confirmation prompts, text input, and selection menus, with --yes for CI.
---
...
`default="..."` is optional and must be one of the listed choices — an invalid default is rejected when the config loads. When set, the interactive cursor starts on the default, and `--yes` selects it non-interactively. Without a default, `--yes` fails with a clear error rather than silently picking an option.
...
- **`choose`** — uses the `default` choice; if the `choose` has no default, the run fails with a clear error (it never silently picks the first option)
...
:::caution[Prompts without defaults]
If a `prompt` has no `default` and `--yes` is used, the variable will be an empty string, and a `choose` without a `default` fails outright under `--yes`. Make sure prompts and menus used in CI have sensible defaults.
:::
