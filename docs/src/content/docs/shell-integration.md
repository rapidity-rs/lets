...
description: Dynamic shell completions that update automatically when your lets.kdl changes.
---
...
Add one line to your shell config (`~/.zshrc`, `~/.bashrc`, or `~/.config/fish/config.fish`):

```sh
eval "$(lets self setup)"
```

`lets self setup` detects your shell and emits the correct completion setup line. You can also see what it produces, or write the raw line yourself:
...
Restart your shell (or run `exec $SHELL`), then try pressing <kbd>Tab</kbd> after `lets `.
...
:::caution[Static completions require regeneration]
